...
Bevor Sie dieses Verfahren befolgen, sollten Sie wissen, link:../concepts/concept_solidfire_concepts_volumes.html#persistent-volumes["Persistente Volumes"]ob Sie diese verwenden möchten oder nicht. Persistente Volumes sind optional, jedoch im Falle eines Datenverlusts bei der Management-Node-Konfiguration empfohlen.
